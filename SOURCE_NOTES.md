...
**Data-quality hold, now localized to one entry.** Directly subtracting the SI's labeled absolute methane and ethynyl energies from its methane transition-structure energy gives approximately −14.76 kcal/mol, inconsistent with the main paper's positive barrier.

That inconsistency has since been traced. Recomputing every supplied species at all-electron CCSD(T)/cc-pVDZ on the SI's own geometries reproduces four of the five labeled energies to under 1×10⁻⁷ hartree: the transition structure, ethynyl radical, acetylene and methyl radical. Methane is the only entry that does not reproduce, and it is short by 17.15 kcal/mol. Substituting the recomputed methane energy into the otherwise unchanged SI set gives +2.3986 kcal/mol, matching the main paper's own Table 4 value of 2.4. The SI methane entry therefore behaves like a transcription or tabulation error rather than a different calculation; no correction has been sought from the authors, and none is assumed here.

The recomputation is reproducible: `python data/validation/si-energy-reproduction/reproduce.py`, with results and per-species SCF diagnostics in `data/validation/si-energy-reproduction/si-energy-reproduction.json`.

**Reaching this required an SCF initial-guess scan, and omitting it silently produces wrong answers.** The ethynyl radical has at least two converged, stable UHF solutions at the supplied geometry, 8.7 kcal/mol apart; PySCF's default `minao` guess finds the higher one, and PySCF's own stability analysis reports both as stable. A CCSD(T) energy built on the higher solution is 14.3 kcal/mol too high, which initially made the correct published ethynyl entry appear erroneous. The supplied transition structure likewise admits multiple solutions, spread 22.9 kcal/mol. Any open-shell Hartree-Fock result in this repository must therefore scan initial guesses and retain the lowest solution. The DFT path was checked separately and is guess-independent for all five species — **but do not generalise that**. It holds for these five species at these geometries and does not hold for DFT in general: propynyl at PBE0-D3(BJ)/def2-SVP has the default `minao` guess converging 11.2 kcal/mol above the `atom`/`huckel`/`1e` solution. Scan guesses for open-shell density-functional work too, at every new geometry.

A related trap, seen in every case examined so far: **spin contamination does not identify the correct solution, and the obvious heuristic is backwards.** For the ethynyl radical (UHF), the methane transition structure (UHF) and propynyl (PBE0-D3), the *cleaner* S² belonged to the higher, wrong solution each time. Choosing by lowest S² would have picked wrong three times out of three.

The individual SI absolute energies still should not be used as acceptance targets without this check. Geometry provenance remains useful independently, and the stationary-structure caveats above are unaffected: the collinear methane transition structure carries three imaginary modes in the paper's own Table 1 and is not a verified first-order saddle.
