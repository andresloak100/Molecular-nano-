...
TUNNELLING = Term(
    ...
    "a non-tunnelling explanation. THAT OPEN END IS NOW CLOSED, by the mechanism "
    "guessed rather than by tunnelling. Canonical TST gives Ea_app = dE0 + RT + <E_TS> "
    "- <E_CH4> - <E_C2H>. Counting translation and rotation only, and noting C2H is "
    "LINEAR so it carries 2.5 RT against 3.0 for the nonlinear species, the bracket is "
    "-2.5 RT and Ea_app = dE0 - 1.5 RT. At the 256 K mean that is 0.763, predicting "
    "0.937 against a measured 0.976, a difference of 0.039 kcal/mol. Standard thermal "
    "averaging accounts for essentially the whole gap with no tunnelling at all. "
    "RESTRAINT: that agreement is better than the approximation deserves, since "
    "vibrational thermal energy at the forming complex is nonzero and would raise the "
    "prediction, and stacking the external lane's 16 percent tunnelling contribution on "
    "top gives 0.821, which undershoots. The terms do not simply add, so this is not a "
    "validated decomposition. The defensible claim is narrower and still useful: the gap "
    "does NOT require large tunnelling, because an independent and unavoidable effect of "
    "the right size already explains it. Weak further support for the low branch; not a "
    "measurement of |nu|. Status remains OPEN pending a verified |nu|. ""depend on its precision. The term stays unmeasured until a verified saddle supplies "
